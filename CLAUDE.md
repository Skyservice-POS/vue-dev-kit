...
`npm test` (vitest + @vue/test-utils + happy-dom), `npm run test:watch` під час роботи.
Тести лежать **поряд з компонентом** — `src/shared/ui/X/X.test.ts`.

- `vitest.config.ts` окремий від `vite.config.ts`: у другому живуть lib-збірка й
  `dts`, яким у тестах нема чого робити.
- У `vite.config.ts` тестові файли **виключені з `dts`** (`exclude: ['src/**/*.test.ts']`).
  Без цього кожен `*.test.ts` їхав би у пакет своїм `.d.ts`.
- happy-dom віддає нулі з `getBoundingClientRect()`, тож усе, що рахує позицію
  (`updatePlacement` у `SkySelectSearch`, `SkyFilterDropdown`), треба тестувати
  зі **зістабленим боксом** і заданим `window.innerHeight`.

**Тест без зубів гірший за відсутній.** Написавши тест на поведінку, зламай цю
поведінку в компоненті й переконайся, що тест упав, — потім поверни. На
`SkySelectSearch` так перевірено чотири мутації: прибрати фліп, прибрати
переоцінку на скрол, прибрати стелю висоти, емітити `String(value)`.

## Локальна перевірка змін у застосунку

Щоб подивитись зміну кіта в міні-застосунку — `npm link` (або `file:` залежність),
**не копіювати `dist/` руками в його `node_modules`**.

Копіювання проходить повз npm, тому Vite не переоптимізовує свій пребандл: у
`node_modules/.vite/deps/` лишається **стара** збірка компонента зі старим
scope id (`data-v-…`), а `style.css` уже новий. Атрибути не збігаються — і всі
scoped-стилі компонента мовчки перестають діяти. Виглядає як «зламалась верстка»,
хоча код цілий.

Якщо все ж скопіював — чистити `node_modules/.vite` і перезапускати дев-сервер
(або `vite --force`).
