...
Блок футера з'являється, **тільки** якщо передано слот `#footer`. Без нього тіло займає весь простір, і знизу додається запасний відступ `15px` (змінна `--sky-modal-body-padding-bottom-no-footer`), щоб останні елементи не впирались у край модалки.
:::
